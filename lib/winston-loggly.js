...
const validateLogdata = (event) => {
  if (event == null) {
    return {};
  } else if (typeof event !== 'object') {
    return { logdata: event };
  } else {
    return isEventErrorObject(event) ?
      convertErrorObjectToJSON(event) :
      Object.assign(event || {})
  }
}

function isEventErrorObject(event) {
  return event && event.level && event.stack && event.message;
}

function convertErrorObjectToJSON(event) {
  return {
    level: event.level,
    stack: event.stack,
...
//
// ### function log (logMessage, callback)
// #### @logMessage contains-
...
//
Loggly.prototype.log = function (logMessage, callback) {

  const message = validateLogdata(logMessage);
  ...
  message.level = logMessage.level;
  message.message = logMessage.message || message.message;
  ...
  return (logMessage && logMessage.tags)
    ? this.client.log(message, logMessage.tags, logged)
